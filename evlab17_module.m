    ...
    case {'init','initforce'} % misc initialization checks
        if strcmpi(option,'initforce') || isempty(done_init) || ~(isequal(done_init,0)||isequal(done_init,path))
            ...
            if isempty(dir(verfilename_SPM)),
                fprintf('Path file %s not found. Using implicit SPM paths\n',verfilename_SPM)
                folderOptions_spm={fullfile(fileparts(fileparts(which(mfilename))),'spm12'),'/software/spm12'}; % folders where to search for SPM12
                FORCEVER_SPM=false;
            else
            ...
            if isempty(dir(verfilename_CONN)),
                fprintf('Path file %s not found. Using implicit CONN paths\n',verfilename_CONN)
                folderOptions_conn={fullfile(fileparts(fileparts(which(mfilename))),'conn'),'/software/conn'}; % folders where to search for CONN
                FORCEVER_CONN=false;
            else
            ...
            if isempty(dir(verfilename_SPM_SS)),
                fprintf('Path file %s not found. Using implicit SPM_SS paths\n',verfilename_SPM_SS)
                folderOptions_spm_ss={fullfile(fileparts(fileparts(which(mfilename))),'spm_ss'),'/software/spm_ss'}; % folders where to search for SPM_SS
            ...
            if isempty(dir(verfilename_SPM)), if ~FORCEVER_SPM, fprintf('Whitelist file %s not found. Skipping SPM version/release check\n',verfilename_SPM); end
            else
                ver_SPM=spm('version');
            ...
            if isempty(dir(verfilename_CONN)), if ~FORCEVER_CONN, fprintf('Whitelist file %s not found. Skipping CONN version/release check\n',verfilename_CONN); end
            else
            ...
            if isempty(dir(verfilename_SPM_SS)), if ~FORCEVER_SPM_SS, fprintf('Whitelist file %s not found. Skipping SPM_SS version/release check\n',verfilename_SPM_SS); end
            else
                try, 
            ...
            if ~isempty(isempty(which('spm_ss'))), conn_checkdistributionfiles spm_ss silent; end
            done_init=path;
        end
